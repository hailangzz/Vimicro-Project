...
trainfiles = os.listdir(r"D:\迅雷下载\AI数据集汇总\汽车拍照检测识别\CCPD2019\ccpd_base")  #（图片文件夹）
...
num = 0
trainDir = r"D:\迅雷下载\AI数据集汇总\汽车拍照检测识别\CCPD_COCO_dataset\train\images"   #（将图片文件夹中的6份放在这个文件夹下）
...
for i in index_list:
    fileName = os.path.join(r"D:\迅雷下载\AI数据集汇总\汽车拍照检测识别\CCPD2019\ccpd_base", trainfiles[i])  #（图片文件夹）+图片名=图片地址
    if num < num_train*0.8:  # 7:1:2
        # print(str(fileName))
