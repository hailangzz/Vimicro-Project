...
@torch.no_grad()
def run(weights='weights/yolov5s.pt',  # 权重文件地址 默认 weights/yolov5.pt
    ...
    for path, img, im0s, vid_cap in dataset:
        # gt_label = getGtLabelByImagePath([path], im0s.shape)
        # 5.1、处理每一张图片的格式
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            #     cls = int(gt[0])
            #     label = gt[1:]
            #     text = None if hide_labels else (names[cls])
            #     # 画出gt框
            #     plot_one_box(label, im0, label=text, color=(0, 0, 255), line_thickness=4)
...
def parse_opt():
    """
    opt参数解析
    weights: 模型的权重地址 默认 weights/yolov5.pt
