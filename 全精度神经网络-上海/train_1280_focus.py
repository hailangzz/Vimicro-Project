...
from rgb2yuv.RGB2YUV_conv_RGB2YUV420p import Net
...
def train(hyp, opt, model_rgb2yuv,device, callbacks):  # hyp is path/to/hyp.yaml or hyp dictionary
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            imgs, imgs_uv=model_rgb2yuv(imgs)
            # Warmup
            if ni <= nw:
                xi = [0, nw]  # x interp
...
def parse_opt(known=False):
    ...
    parser.add_argument('--workers', type=int, default=8, help='max dataloader workers (per RANK in DDP mode)')         # num_works
    parser.add_argument('--project', default='runs', help='save to project/name')                                       # 训练结果保存位置
    parser.add_argument('--name', default='exp_1280f_sp3_', help='save to project/name')
...
def main(opt, callbacks=Callbacks()):
    ...
    print('*********',device)
    model_rgb2yuv = Net()
    model_rgb2yuv.load_state_dict(torch.load('rgb2yuv/RGB2YUV420p_1_new.pth'))
    model_rgb2yuv = model_rgb2yuv.to(device)
    model_rgb2yuv.eval()

    # Train
    if not opt.evolve:
        train(opt.hyp, opt, model_rgb2yuv,device, callbacks)
